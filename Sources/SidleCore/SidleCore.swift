...
public extension WordList {
    func filterMatching(_ pattern: String) throws -> WordList {
...
extension WordList {
    func filter(with fact: Fact) throws -> WordList{
        ...
        case .minimumOccurrenceCount(let char, let count):
            return try filterMatching(Array(repeating: char, count: count).map(String.init).joined(separator: ".*"))
        default:
            return self
        }
    }
    
    public func filter<F: Collection>(with facts: F, wordLength: Int) throws -> WordList where F.Element == Fact {
